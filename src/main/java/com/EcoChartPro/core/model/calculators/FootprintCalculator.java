...
public class FootprintCalculator {
    ...
    /**
     * Calculates historical footprint bars by approximating volume distribution from K-lines.
    ...
    public void calculateHistoricalFootprints(List<KLine> klineHistory) {
        ...
        logger.info("Approximated footprint data for {} historical candles with a price step of {}.", klineHistory.size(), lastCalculatedPriceStep);
    }

    /**
     * Updates the currently forming footprint bar with a live trade tick.
     * @param tick The incoming trade data.
     * @param currentlyFormingCandle The K-line that this tick belongs to.
     */
    ...
    private void calculateDynamicPriceStep(List<KLine> klineHistory) {
        if (klineHistory.size() > 1) {
            ...
            this.lastCalculatedPriceStep = (dynamicPriceStep.compareTo(BigDecimal.ZERO) > 0) ? dynamicPriceStep : new BigDecimal("0.00001");
        } else {
